...
class CodeReduction(nn.Module):
    def __init__(self, c_dim, feat_c, feat_hw, prob=True):
        ...
        self.main = nn.Sequential(
            spectral_norm(nn.Conv2d(feat_c, c_dim, 3, 1, 1, bias=True, groups=1)),
            nn.LeakyReLU(0.1),
...
class UpConvBlock(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        modules = [spectral_norm(nn.Conv2d(in_channel, out_channel, 3, 1, 1, bias=True)),
...
class OOGANInput(nn.Module):
    ...
    def __init__(self, c_dim, z_dim, feat_4, feat_8):
        ...
        self.from_c_4 = nn.Sequential(
            UnFlatten(1), 
            spectral_norm(nn.ConvTranspose2d(c_dim, feat_4, 4, 1, 0, bias=True)),
        ...
        if z_dim > 0:
            ...
            self.from_z_8 = nn.Sequential(
                UnFlatten(1), 
                spectral_norm(nn.ConvTranspose2d(z_dim, z_dim//2, 4, 1, 0, bias=True)),
                nn.BatchNorm2d(z_dim//2), 
                nn.LeakyReLU(0.01),
                spectral_norm(nn.ConvTranspose2d(z_dim//2, 1, 4, 2, 1, bias=True)),
                nn.BatchNorm2d(1), 
                nn.LeakyReLU(0.01)
                )
...
class InfoGANInput(nn.Module):
    def __init__(self, c_dim, z_dim, feat_4, feat_8):
        ...
        self.main = nn.Sequential(
            UnFlatten(1), 
            spectral_norm(nn.ConvTranspose2d(c_dim+z_dim, feat_4, 4, 1, 0, bias=True)),
            nn.BatchNorm2d(feat_4), 
            nn.LeakyReLU(0.01),
