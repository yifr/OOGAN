...
training_config_celeba = {
    ...
    "DATA_ROOT": "/media/bingchen/database/celebA/", # follow the pytorch's torchvision.dataset.Imagefolder's path format
    "DATALOADER_WORKERS":8, # pytorch multi-thread cpu data loading config
    "NOISE_ANNEL": False, # usually not necessary
    ...
    "C_DIM": 16, # the control vector c dimension, usually 100 on 128x128 images
    "IM_SIZE": 128, # can choose from {32, 64, 128, 256} 
    "LR": 1e-4, # usually no need to change it
    "LAMBDA": 1, # controls the weights of the mutual information part in OOGAN's loss, usually just set to 1 is fine
    "GAMMA": 1,  # controls the weights of the one-hot trick part in OOGAN's loss, usually just set to 1 is fine
    "TRIAL_NAME": 'trial_celeba_oogan_1' # it determines the save folder's name, please make sure to set a new name when you start a new training
}
...
training_config = training_config_dsprites